...
try {
  ...
  assert.equal(
    await page.locator(".map-header-actions #project-progress + #open-progress").count(),
    1,
  );
  ...
  const actionsBox = await page.locator(".map-header-actions").boundingBox();
  assert(
    actionsBox.x >= headerBox.x &&
