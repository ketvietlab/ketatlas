...
Project-wide verified/checklist percentages and blocker counts appear in the top bar beside **Screens**. The sidebar shows **Checks %** after each workflow’s step count (`2 steps [30%]`): completed recorded acceptance checks divided by all recorded checks for its unique screens. Repeated nodes and error variants count once; note/external nodes do not count. Screenless flows have no badge. Flows with no recorded checks show **—**, not a measured 0%. Percentages round down so unfinished checks cannot appear as 100%.
